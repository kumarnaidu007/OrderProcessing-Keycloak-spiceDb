...
public class OrdersController : ControllerBase
{
    ...
    {
        _logger.LogInformation("Create order requested.");
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new { message = "Unexpected error while creating order." });
        }
    }
    ...
    public async Task<ActionResult<OrderResponse>> Get(long orderId, CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "Error while fetching order {OrderId}.", orderId);
            return StatusCode(500, new { message = "Unexpected error while fetching order." });
        }
    }
    ...
    public async Task<ActionResult<OrderStatusResponse>> GetStatus(long orderId, CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "Error while fetching status for order {OrderId}.", orderId);
            return StatusCode(500, new { message = "Unexpected error while fetching order status." });
        }
    }
    ...
    {
        _logger.LogInformation("Cancel requested for order {OrderId}.", orderId);
        await using var tx = await _db.Database.BeginTransactionAsync(ct);
        try
        {
        ...
        {
            await tx.RollbackAsync(ct);
            _logger.LogError(ex, "Error while cancelling order {OrderId}.", orderId);
            return StatusCode(500, new { message = "Unexpected error while cancelling order." });
        }
    }
        ...
        CancellationToken ct = default)
    {
        ...
        {
            _logger.LogError(ex, "Error while listing orders.");
            return StatusCode(500, new { message = "Unexpected error while listing orders." });
        }
    }
